.class public abstract Lcom/android/wm/shell/transition/TransitionAnimationHelper;
.super Ljava/lang/Object;
.source "go/retraceme af8e0b46c0cb0ee2c99e9b6d0c434e5c0b686fd9230eaab7fb9a40e3a9d0cf6f"


# direct methods
.method public static getTransitionTypeFromInfo(Landroid/window/TransitionInfo;)I
    .locals 5

    .line 1
    invoke-virtual {p0}, Landroid/window/TransitionInfo;->getType()I

    .line 2
    .line 3
    .line 4
    move-result v0

    .line 5
    const/16 v1, 0xd

    .line 6
    .line 7
    const/4 v2, 0x2

    .line 8
    const/4 v3, 0x1

    .line 9
    if-eq v0, v1, :cond_0

    .line 10
    .line 11
    const/16 v1, 0xe

    .line 12
    .line 13
    if-ne v0, v1, :cond_2

    .line 14
    .line 15
    :cond_0
    invoke-virtual {p0}, Landroid/window/TransitionInfo;->getChanges()Ljava/util/List;

    .line 16
    .line 17
    .line 18
    move-result-object v1

    .line 19
    invoke-interface {v1}, Ljava/util/List;->isEmpty()Z

    .line 20
    .line 21
    .line 22
    move-result v1

    .line 23
    if-nez v1, :cond_2

    .line 24
    .line 25
    invoke-virtual {p0}, Landroid/window/TransitionInfo;->getChanges()Ljava/util/List;

    .line 26
    .line 27
    .line 28
    move-result-object p0

    .line 29
    const/4 v0, 0x0

    .line 30
    invoke-interface {p0, v0}, Ljava/util/List;->get(I)Ljava/lang/Object;

    .line 31
    .line 32
    .line 33
    move-result-object p0

    .line 34
    check-cast p0, Landroid/window/TransitionInfo$Change;

    .line 35
    .line 36
    invoke-virtual {p0}, Landroid/window/TransitionInfo$Change;->getMode()I

    .line 37
    .line 38
    .line 39
    move-result p0

    .line 40
    invoke-static {p0}, Lcom/android/wm/shell/shared/TransitionUtil;->isOpeningMode(I)Z

    .line 41
    .line 42
    .line 43
    move-result p0

    .line 44
    if-eqz p0, :cond_1

    .line 45
    .line 46
    return v3

    .line 47
    :cond_1
    return v2

    .line 48
    :cond_2
    if-ne v0, v3, :cond_8

    .line 49
    .line 50
    invoke-virtual {p0}, Landroid/window/TransitionInfo;->getChanges()Ljava/util/List;

    .line 51
    .line 52
    .line 53
    move-result-object p0

    .line 54
    invoke-interface {p0}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    .line 55
    .line 56
    .line 57
    move-result-object p0

    .line 58
    :cond_3
    :goto_0
    invoke-interface {p0}, Ljava/util/Iterator;->hasNext()Z

    .line 59
    .line 60
    .line 61
    move-result v1

    .line 62
    if-eqz v1, :cond_7

    .line 63
    .line 64
    invoke-interface {p0}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    .line 65
    .line 66
    .line 67
    move-result-object v1

    .line 68
    check-cast v1, Landroid/window/TransitionInfo$Change;

    .line 69
    .line 70
    invoke-virtual {v1}, Landroid/window/TransitionInfo$Change;->getTaskInfo()Landroid/app/ActivityManager$RunningTaskInfo;

    .line 71
    .line 72
    .line 73
    move-result-object v4

    .line 74
    if-nez v4, :cond_4

    .line 75
    .line 76
    const/16 v4, 0x20

    .line 77
    .line 78
    invoke-virtual {v1, v4}, Landroid/window/TransitionInfo$Change;->hasFlags(I)Z

    .line 79
    .line 80
    .line 81
    move-result v4

    .line 82
    if-eqz v4, :cond_5

    .line 83
    .line 84
    :cond_4
    invoke-static {v1}, Lcom/android/wm/shell/shared/TransitionUtil;->isOrderOnly(Landroid/window/TransitionInfo$Change;)Z

    .line 85
    .line 86
    .line 87
    move-result v4

    .line 88
    if-nez v4, :cond_5

    .line 89
    .line 90
    goto :goto_1

    .line 91
    :cond_5
    invoke-virtual {v1}, Landroid/window/TransitionInfo$Change;->getTaskInfo()Landroid/app/ActivityManager$RunningTaskInfo;

    .line 92
    .line 93
    .line 94
    move-result-object v4

    .line 95
    if-eqz v4, :cond_6

    .line 96
    .line 97
    const v4, 0x10122

    .line 98
    .line 99
    .line 100
    invoke-virtual {v1, v4}, Landroid/window/TransitionInfo$Change;->hasFlags(I)Z

    .line 101
    .line 102
    .line 103
    move-result v4

    .line 104
    if-eqz v4, :cond_6

    .line 105
    .line 106
    goto :goto_0

    .line 107
    :cond_6
    invoke-virtual {v1}, Landroid/window/TransitionInfo$Change;->getMode()I

    .line 108
    .line 109
    .line 110
    move-result v1

    .line 111
    if-ne v1, v3, :cond_3

    .line 112
    .line 113
    return v0

    .line 114
    :cond_7
    return v2

    .line 115
    :cond_8
    :goto_1
    return v0
.end method

.method public static isCoveredByOpaqueFullscreenChange(Landroid/window/TransitionInfo$Change;Landroid/window/TransitionInfo;)Z
    .locals 2

    .line 1
    invoke-virtual {p1}, Landroid/window/TransitionInfo;->getChanges()Ljava/util/List;

    .line 2
    .line 3
    .line 4
    move-result-object p1

    .line 5
    invoke-interface {p1}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    .line 6
    .line 7
    .line 8
    move-result-object p1

    .line 9
    :cond_0
    invoke-interface {p1}, Ljava/util/Iterator;->hasNext()Z

    .line 10
    .line 11
    .line 12
    move-result v0

    .line 13
    const/4 v1, 0x0

    .line 14
    if-eqz v0, :cond_2

    .line 15
    .line 16
    invoke-interface {p1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    .line 17
    .line 18
    .line 19
    move-result-object v0

    .line 20
    check-cast v0, Landroid/window/TransitionInfo$Change;

    .line 21
    .line 22
    if-ne v0, p0, :cond_1

    .line 23
    .line 24
    return v1

    .line 25
    :cond_1
    invoke-virtual {v0}, Landroid/window/TransitionInfo$Change;->getFlags()I

    .line 26
    .line 27
    .line 28
    move-result v1

    .line 29
    and-int/lit8 v1, v1, 0x4

    .line 30
    .line 31
    if-nez v1, :cond_0

    .line 32
    .line 33
    invoke-virtual {v0}, Landroid/window/TransitionInfo$Change;->getTaskInfo()Landroid/app/ActivityManager$RunningTaskInfo;

    .line 34
    .line 35
    .line 36
    move-result-object v1

    .line 37
    if-eqz v1, :cond_0

    .line 38
    .line 39
    invoke-virtual {v0}, Landroid/window/TransitionInfo$Change;->getTaskInfo()Landroid/app/ActivityManager$RunningTaskInfo;

    .line 40
    .line 41
    .line 42
    move-result-object v0

    .line 43
    invoke-virtual {v0}, Landroid/app/ActivityManager$RunningTaskInfo;->getWindowingMode()I

    .line 44
    .line 45
    .line 46
    move-result v0

    .line 47
    const/4 v1, 0x1

    .line 48
    if-ne v0, v1, :cond_0

    .line 49
    .line 50
    :cond_2
    return v1
.end method

.method public static loadAttributeAnimation(ILandroid/window/TransitionInfo;Landroid/window/TransitionInfo$Change;ILcom/android/internal/policy/TransitionAnimation;Z)Landroid/view/animation/Animation;
    .locals 18

    .line 1
    move/from16 v0, p0

    .line 2
    .line 3
    move/from16 v1, p3

    .line 4
    .line 5
    move-object/from16 v2, p4

    .line 6
    .line 7
    invoke-virtual/range {p2 .. p2}, Landroid/window/TransitionInfo$Change;->getMode()I

    .line 8
    .line 9
    .line 10
    move-result v3

    .line 11
    invoke-virtual/range {p2 .. p2}, Landroid/window/TransitionInfo$Change;->getFlags()I

    .line 12
    .line 13
    .line 14
    move-result v4

    .line 15
    invoke-static {v3}, Lcom/android/wm/shell/shared/TransitionUtil;->isOpeningType(I)Z

    .line 16
    .line 17
    .line 18
    move-result v3

    .line 19
    invoke-virtual/range {p2 .. p2}, Landroid/window/TransitionInfo$Change;->getTaskInfo()Landroid/app/ActivityManager$RunningTaskInfo;

    .line 20
    .line 21
    .line 22
    move-result-object v5

    .line 23
    const/4 v6, 0x1

    .line 24
    if-eqz v5, :cond_0

    .line 25
    .line 26
    move v5, v6

    .line 27
    goto :goto_0

    .line 28
    :cond_0
    const/4 v5, 0x0

    .line 29
    :goto_0
    if-eqz v5, :cond_1

    .line 30
    .line 31
    invoke-virtual/range {p2 .. p2}, Landroid/window/TransitionInfo$Change;->getTaskInfo()Landroid/app/ActivityManager$RunningTaskInfo;

    .line 32
    .line 33
    .line 34
    move-result-object v8

    .line 35
    invoke-virtual {v8}, Landroid/app/ActivityManager$RunningTaskInfo;->isFreeform()Z

    .line 36
    .line 37
    .line 38
    move-result v8

    .line 39
    if-eqz v8, :cond_1

    .line 40
    .line 41
    move v10, v6

    .line 42
    :goto_1
    move-object/from16 v8, p1

    .line 43
    .line 44
    move-object/from16 v9, p2

    .line 45
    .line 46
    goto :goto_2

    .line 47
    :cond_1
    const/4 v10, 0x0

    .line 48
    goto :goto_1

    .line 49
    :goto_2
    invoke-static {v9, v8}, Lcom/android/wm/shell/transition/TransitionAnimationHelper;->isCoveredByOpaqueFullscreenChange(Landroid/window/TransitionInfo$Change;Landroid/window/TransitionInfo;)Z

    .line 50
    .line 51
    .line 52
    move-result v8

    .line 53
    invoke-virtual {v9}, Landroid/window/TransitionInfo$Change;->getAnimationOptions()Landroid/window/TransitionInfo$AnimationOptions;

    .line 54
    .line 55
    .line 56
    move-result-object v11

    .line 57
    if-eqz v11, :cond_2

    .line 58
    .line 59
    invoke-virtual {v11}, Landroid/window/TransitionInfo$AnimationOptions;->getType()I

    .line 60
    .line 61
    .line 62
    move-result v12

    .line 63
    goto :goto_3

    .line 64
    :cond_2
    const/4 v12, 0x0

    .line 65
    :goto_3
    const/16 v13, 0xe

    .line 66
    .line 67
    const/4 v15, 0x5

    .line 68
    const/16 v16, 0x0

    .line 69
    .line 70
    const/4 v7, 0x2

    .line 71
    const/4 v14, 0x4

    .line 72
    if-eqz p5, :cond_6

    .line 73
    .line 74
    if-ne v0, v6, :cond_4

    .line 75
    .line 76
    if-eqz v3, :cond_3

    .line 77
    .line 78
    const/16 v1, 0x1c

    .line 79
    .line 80
    goto :goto_4

    .line 81
    :cond_3
    const/16 v1, 0x1d

    .line 82
    .line 83
    :goto_4
    move/from16 v7, v16

    .line 84
    .line 85
    goto/16 :goto_a

    .line 86
    .line 87
    :cond_4
    if-ne v0, v7, :cond_21

    .line 88
    .line 89
    if-eqz v3, :cond_5

    .line 90
    .line 91
    move/from16 v1, v16

    .line 92
    .line 93
    goto :goto_4

    .line 94
    :cond_5
    const/16 v1, 0x1b

    .line 95
    .line 96
    goto :goto_4

    .line 97
    :cond_6
    if-ne v1, v14, :cond_8

    .line 98
    .line 99
    if-eqz v3, :cond_7

    .line 100
    .line 101
    const/16 v1, 0x14

    .line 102
    .line 103
    goto :goto_4

    .line 104
    :cond_7
    const/16 v1, 0x15

    .line 105
    .line 106
    goto :goto_4

    .line 107
    :cond_8
    if-ne v1, v15, :cond_a

    .line 108
    .line 109
    if-eqz v3, :cond_9

    .line 110
    .line 111
    const/16 v1, 0x16

    .line 112
    .line 113
    goto :goto_4

    .line 114
    :cond_9
    const/16 v1, 0x17

    .line 115
    .line 116
    goto :goto_4

    .line 117
    :cond_a
    if-ne v1, v7, :cond_c

    .line 118
    .line 119
    if-eqz v3, :cond_b

    .line 120
    .line 121
    const/16 v1, 0x10

    .line 122
    .line 123
    goto :goto_4

    .line 124
    :cond_b
    const/16 v1, 0x11

    .line 125
    .line 126
    goto :goto_4

    .line 127
    :cond_c
    const/4 v15, 0x3

    .line 128
    if-ne v1, v15, :cond_e

    .line 129
    .line 130
    if-eqz v3, :cond_d

    .line 131
    .line 132
    const/16 v1, 0x12

    .line 133
    .line 134
    goto :goto_4

    .line 135
    :cond_d
    const/16 v1, 0x13

    .line 136
    .line 137
    goto :goto_4

    .line 138
    :cond_e
    if-nez v8, :cond_10

    .line 139
    .line 140
    if-eqz v10, :cond_10

    .line 141
    .line 142
    invoke-static {v0}, Lcom/android/wm/shell/shared/TransitionUtil;->isOpeningMode(I)Z

    .line 143
    .line 144
    .line 145
    move-result v1

    .line 146
    if-eqz v1, :cond_10

    .line 147
    .line 148
    invoke-virtual {v9}, Landroid/window/TransitionInfo$Change;->getMode()I

    .line 149
    .line 150
    .line 151
    move-result v1

    .line 152
    if-ne v1, v14, :cond_10

    .line 153
    .line 154
    and-int/lit8 v1, v4, 0x4

    .line 155
    .line 156
    if-eqz v1, :cond_f

    .line 157
    .line 158
    move v1, v6

    .line 159
    goto :goto_5

    .line 160
    :cond_f
    move/from16 v1, v16

    .line 161
    .line 162
    :goto_5
    move v7, v1

    .line 163
    const/4 v1, 0x7

    .line 164
    goto/16 :goto_a

    .line 165
    .line 166
    :cond_10
    if-nez v8, :cond_12

    .line 167
    .line 168
    if-eqz v10, :cond_12

    .line 169
    .line 170
    if-ne v0, v15, :cond_12

    .line 171
    .line 172
    invoke-virtual {v9}, Landroid/window/TransitionInfo$Change;->getMode()I

    .line 173
    .line 174
    .line 175
    move-result v1

    .line 176
    if-ne v1, v15, :cond_12

    .line 177
    .line 178
    and-int/lit8 v1, v4, 0x4

    .line 179
    .line 180
    if-eqz v1, :cond_11

    .line 181
    .line 182
    move v1, v6

    .line 183
    goto :goto_6

    .line 184
    :cond_11
    move/from16 v1, v16

    .line 185
    .line 186
    :goto_6
    move v7, v1

    .line 187
    move v1, v14

    .line 188
    goto/16 :goto_a

    .line 189
    .line 190
    :cond_12
    if-ne v0, v6, :cond_18

    .line 191
    .line 192
    and-int/lit8 v1, v4, 0x4

    .line 193
    .line 194
    if-eqz v1, :cond_13

    .line 195
    .line 196
    move v1, v6

    .line 197
    goto :goto_7

    .line 198
    :cond_13
    move/from16 v1, v16

    .line 199
    .line 200
    :goto_7
    if-eqz v5, :cond_14

    .line 201
    .line 202
    if-eqz v1, :cond_14

    .line 203
    .line 204
    if-nez v3, :cond_14

    .line 205
    .line 206
    goto :goto_5

    .line 207
    :cond_14
    if-eqz v5, :cond_16

    .line 208
    .line 209
    if-nez v1, :cond_16

    .line 210
    .line 211
    if-eqz v3, :cond_15

    .line 212
    .line 213
    const/16 v7, 0x8

    .line 214
    .line 215
    goto :goto_8

    .line 216
    :cond_15
    const/16 v7, 0x9

    .line 217
    .line 218
    :goto_8
    move/from16 v17, v7

    .line 219
    .line 220
    move v7, v1

    .line 221
    move/from16 v1, v17

    .line 222
    .line 223
    goto :goto_a

    .line 224
    :cond_16
    if-eqz v3, :cond_17

    .line 225
    .line 226
    move v7, v14

    .line 227
    goto :goto_8

    .line 228
    :cond_17
    const/4 v7, 0x5

    .line 229
    goto :goto_8

    .line 230
    :cond_18
    if-ne v0, v15, :cond_1a

    .line 231
    .line 232
    if-eqz v3, :cond_19

    .line 233
    .line 234
    const/16 v1, 0xc

    .line 235
    .line 236
    goto/16 :goto_4

    .line 237
    .line 238
    :cond_19
    const/16 v1, 0xd

    .line 239
    .line 240
    goto/16 :goto_4

    .line 241
    .line 242
    :cond_1a
    if-ne v0, v7, :cond_1f

    .line 243
    .line 244
    and-int/lit8 v1, v4, 0x4

    .line 245
    .line 246
    if-eqz v1, :cond_1b

    .line 247
    .line 248
    if-nez v3, :cond_1b

    .line 249
    .line 250
    move v1, v6

    .line 251
    goto :goto_9

    .line 252
    :cond_1b
    move/from16 v1, v16

    .line 253
    .line 254
    :goto_9
    if-eqz v5, :cond_1d

    .line 255
    .line 256
    if-nez v1, :cond_1d

    .line 257
    .line 258
    if-eqz v3, :cond_1c

    .line 259
    .line 260
    const/16 v7, 0xa

    .line 261
    .line 262
    goto :goto_8

    .line 263
    :cond_1c
    const/16 v7, 0xb

    .line 264
    .line 265
    goto :goto_8

    .line 266
    :cond_1d
    if-eqz v3, :cond_1e

    .line 267
    .line 268
    const/4 v7, 0x6

    .line 269
    goto :goto_8

    .line 270
    :cond_1e
    const/4 v7, 0x7

    .line 271
    goto :goto_8

    .line 272
    :cond_1f
    if-ne v0, v14, :cond_21

    .line 273
    .line 274
    if-eqz v3, :cond_20

    .line 275
    .line 276
    move v1, v13

    .line 277
    goto/16 :goto_4

    .line 278
    .line 279
    :cond_20
    const/16 v1, 0xf

    .line 280
    .line 281
    goto/16 :goto_4

    .line 282
    .line 283
    :cond_21
    move/from16 v1, v16

    .line 284
    .line 285
    move v7, v1

    .line 286
    :goto_a
    const/4 v8, 0x0

    .line 287
    if-eqz v1, :cond_29

    .line 288
    .line 289
    if-ne v12, v13, :cond_27

    .line 290
    .line 291
    if-nez v5, :cond_27

    .line 292
    .line 293
    if-eq v1, v14, :cond_23

    .line 294
    .line 295
    const/4 v4, 0x5

    .line 296
    if-eq v1, v4, :cond_23

    .line 297
    .line 298
    const/4 v4, 0x6

    .line 299
    if-eq v1, v4, :cond_22

    .line 300
    .line 301
    const/4 v4, 0x7

    .line 302
    if-eq v1, v4, :cond_22

    .line 303
    .line 304
    goto :goto_b

    .line 305
    :cond_22
    move/from16 v6, v16

    .line 306
    .line 307
    :cond_23
    invoke-virtual {v11, v6}, Landroid/window/TransitionInfo$AnimationOptions;->getCustomActivityTransition(Z)Landroid/window/TransitionInfo$AnimationOptions$CustomActivityTransition;

    .line 308
    .line 309
    .line 310
    move-result-object v8

    .line 311
    :goto_b
    if-eqz v8, :cond_26

    .line 312
    .line 313
    invoke-virtual {v11}, Landroid/window/TransitionInfo$AnimationOptions;->getPackageName()Ljava/lang/String;

    .line 314
    .line 315
    .line 316
    move-result-object v4

    .line 317
    if-eqz v3, :cond_24

    .line 318
    .line 319
    invoke-virtual {v8}, Landroid/window/TransitionInfo$AnimationOptions$CustomActivityTransition;->getCustomEnterResId()I

    .line 320
    .line 321
    .line 322
    move-result v5

    .line 323
    goto :goto_c

    .line 324
    :cond_24
    invoke-virtual {v8}, Landroid/window/TransitionInfo$AnimationOptions$CustomActivityTransition;->getCustomExitResId()I

    .line 325
    .line 326
    .line 327
    move-result v5

    .line 328
    :goto_c
    invoke-virtual {v2, v4, v5}, Lcom/android/internal/policy/TransitionAnimation;->loadAppTransitionAnimation(Ljava/lang/String;I)Landroid/view/animation/Animation;

    .line 329
    .line 330
    .line 331
    move-result-object v2

    .line 332
    if-eqz v2, :cond_25

    .line 333
    .line 334
    invoke-virtual {v8}, Landroid/window/TransitionInfo$AnimationOptions$CustomActivityTransition;->getCustomBackgroundColor()I

    .line 335
    .line 336
    .line 337
    move-result v4

    .line 338
    if-eqz v4, :cond_25

    .line 339
    .line 340
    invoke-virtual {v8}, Landroid/window/TransitionInfo$AnimationOptions$CustomActivityTransition;->getCustomBackgroundColor()I

    .line 341
    .line 342
    .line 343
    move-result v4

    .line 344
    invoke-virtual {v2, v4}, Landroid/view/animation/Animation;->setBackdropColor(I)V

    .line 345
    .line 346
    .line 347
    :cond_25
    :goto_d
    move-object v8, v2

    .line 348
    goto :goto_e

    .line 349
    :cond_26
    invoke-virtual {v11}, Landroid/window/TransitionInfo$AnimationOptions;->getPackageName()Ljava/lang/String;

    .line 350
    .line 351
    .line 352
    move-result-object v4

    .line 353
    invoke-virtual {v11}, Landroid/window/TransitionInfo$AnimationOptions;->getAnimations()I

    .line 354
    .line 355
    .line 356
    move-result v5

    .line 357
    invoke-virtual {v2, v4, v5, v1, v7}, Lcom/android/internal/policy/TransitionAnimation;->loadAnimationAttr(Ljava/lang/String;IIZ)Landroid/view/animation/Animation;

    .line 358
    .line 359
    .line 360
    move-result-object v2

    .line 361
    goto :goto_d

    .line 362
    :cond_27
    if-eqz v7, :cond_28

    .line 363
    .line 364
    if-nez v5, :cond_28

    .line 365
    .line 366
    const v5, 0x10102

    .line 367
    .line 368
    .line 369
    and-int/2addr v4, v5

    .line 370
    if-nez v4, :cond_28

    .line 371
    .line 372
    goto :goto_e

    .line 373
    :cond_28
    invoke-virtual {v2, v1, v7}, Lcom/android/internal/policy/TransitionAnimation;->loadDefaultAnimationAttr(IZ)Landroid/view/animation/Animation;

    .line 374
    .line 375
    .line 376
    move-result-object v8

    .line 377
    :cond_29
    :goto_e
    sget-object v2, Lcom/android/internal/protolog/ProtoLogImpl_767719928$Cache;->WM_SHELL_TRANSITIONS_enabled:[Z

    .line 378
    .line 379
    aget-boolean v2, v2, v16

    .line 380
    .line 381
    if-eqz v2, :cond_2a

    .line 382
    .line 383
    invoke-static {v8}, Ljava/lang/String;->valueOf(Ljava/lang/Object;)Ljava/lang/String;

    .line 384
    .line 385
    .line 386
    move-result-object v2

    .line 387
    int-to-long v4, v1

    .line 388
    invoke-static {v0}, Lcom/android/wm/shell/transition/Transitions;->transitTypeToString(I)Ljava/lang/String;

    .line 389
    .line 390
    .line 391
    move-result-object v0

    .line 392
    invoke-static {v0}, Ljava/lang/String;->valueOf(Ljava/lang/Object;)Ljava/lang/String;

    .line 393
    .line 394
    .line 395
    move-result-object v0

    .line 396
    sget-object v1, Lcom/android/wm/shell/protolog/ShellProtoLogGroup;->WM_SHELL_TRANSITIONS:Lcom/android/wm/shell/protolog/ShellProtoLogGroup;

    .line 397
    .line 398
    invoke-static {v4, v5}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    .line 399
    .line 400
    .line 401
    move-result-object v4

    .line 402
    invoke-static {v3}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    .line 403
    .line 404
    .line 405
    move-result-object v3

    .line 406
    filled-new-array {v2, v4, v0, v3}, [Ljava/lang/Object;

    .line 407
    .line 408
    .line 409
    move-result-object v0

    .line 410
    const-wide v2, -0x47ca01eaa5aec497L    # -6.46301879139433E-38

    .line 411
    .line 412
    .line 413
    .line 414
    .line 415
    const/16 v4, 0xc4

    .line 416
    .line 417
    invoke-static {v1, v2, v3, v4, v0}, Lcom/android/internal/protolog/ProtoLogImpl_767719928;->v(Lcom/android/internal/protolog/common/IProtoLogGroup;JI[Ljava/lang/Object;)V

    .line 418
    .line 419
    .line 420
    :cond_2a
    return-object v8
.end method
